...
import ARTryOnTab from '../components/Stylist/ARTryOnTab';
...
export default function StylistScreen() {
  ...
  const [allWardrobe, setAllWardrobe] = useState<
    { id: string; uri: string; category: string; brand?: string }[]
  >([]);

  const [allOutfits, setAllOutfits] = useState<any[]>([]);

  // 🚀 Eğer Wardrobe'dan "Try it on" ile geldiysek
  useEffect(() => {
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        const cityName =
          geocode[0]?.city || geocode[0]?.subregion || 'Konum';
        ...
        if (data.current_weather) {
          ...
          if (wmoCode === 0) {
            materialIcon = 'weather-sunny';
            iconColor = '#FFCA28';
          } else if (wmoCode >= 1 && wmoCode <= 2) {
            materialIcon = 'weather-partly-cloudy';
            iconColor = '#FFCA28';
          } else if (wmoCode === 3) {
            materialIcon = 'weather-cloudy';
            iconColor = '#90A4AE';
          } else if (wmoCode >= 45 && wmoCode <= 48) {
            materialIcon = 'weather-fog';
            iconColor = '#B0BEC5';
          } else if (wmoCode >= 51 && wmoCode <= 67) {
            materialIcon = 'weather-pouring';
            iconColor = '#4FC3F7';
          } else if (wmoCode >= 71 && wmoCode <= 77) {
            materialIcon = 'weather-snowy';
            iconColor = '#E1F5FE';
          } else if (wmoCode >= 95) {
            materialIcon = 'weather-lightning';
            iconColor = '#FF9800';
          }
      ...
      const fetchRealData = async () => {
        ...
        try {
          const response = await apiClient.get(
            `/clothes/${CURRENT_USER_ID}?size=200`
          );
        ...
        // 🚀 KOMBİNLERİ ÇEK
        try {
          const outfitsRes = await apiClient.get(
            `/outfits/user/${CURRENT_USER_ID}?size=200`
          );

          const rawOutfits =
            outfitsRes.data.content || outfitsRes.data || [];
          ...
          console.error(
            '🚨 Kombinler çekilirken (500) backend hatası:',
            error.message
          );
        }
      };
  ...
  return (
    <View style={[styles.container, { paddingTop: insets.top }]}>
      ...
      <View style={{ flex: 1 }}>
        ...
        {activeTab === 'Canvas' && (
          <CanvasTab allWardrobe={allWardrobe} />
        )}
