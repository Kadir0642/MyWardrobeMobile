...
// 🎨 SIFIR ÇAKIŞMA: MATEMATİKSEL IZGARA (GRID) KOORDİNATLARI
// Ekran 2 Sütun ve 3 Satır olacak şekilde görünmez 6 odacığa bölündü.
const COLLAGE_POSITIONS = [
  { top: '2%', left: '4%', width: '44%', height: '30%' },    // 0: Sol Üst
  { top: '2%', right: '4%', width: '44%', height: '30%' },   // 1: Sağ Üst
  { top: '34%', left: '4%', width: '44%', height: '30%' },   // 2: Sol Orta
  { top: '34%', right: '4%', width: '44%', height: '30%' },  // 3: Sağ Orta
  { top: '66%', left: '4%', width: '44%', height: '30%' },   // 4: Sol Alt
  { top: '66%', right: '4%', width: '44%', height: '30%' }   // 5: Sağ Alt
];
...
export default function AISuggestionsTab({ allWardrobe = [] }: AISuggestionsTabProps) {
  ...
  // 👑 PREMIUM MENÜ STATE'İ (Varsayılan: 0)
  const [activeBlueprintIndex, setActiveBlueprintIndex] = useState<0 | 1 | 2>(0);

  // 🧠 SENİN TASARLADIĞIN 3 KATMANLI ŞABLONLAR
  const AI_BLUEPRINTS = [
    ['Tops', 'Bottoms', 'Footwear', 'Accessories'],                                  // 0: Basic (4 Eşya)
    ['Full_body', 'Footwear', 'Accessories', 'Accessories', 'Accessories'],          // 1: Full Body (5 Eşya)
    ['Outerwear', 'Tops', 'Bottoms', 'Footwear', 'Accessories', 'Accessories']       // 2: Layered (6 Eşya)
  ];
      ...
      <View style={styles.aiOutfitGrid}>
        {isLoading ? (
           ...
           currentOutfit.map((item, index) => {
             // Sırasıyla ızgaradaki odacıklara yerleştiriyoruz
...
const styles = StyleSheet.create({
  ...
  // 🚀 Yeni Izgara Sistemi (Grid) Konteynerleri
